...
describe("team rules", () => {
  it("always creates exactly five teams with four loadout slots", () => {
  ...
  it("moves a used awakener with its complete loadout and swaps occupied slots", () => {
    ...
    teams[1].slots[0].awakenerId = second.id;

    const result = assignEntity(
      teams,
      { kind: "awakener", teamId: "team-2", slotIndex: 0 },
      first.id,
    );

    expect(result.ok).toBe(true);
    expect(result.moved).toBe(true);
    expect(result.teams[1].slots[0]).toEqual({
      awakenerId: first.id,
      wheelIds: [wheel.id, null],
      covenantId: covenant.id,
    });
  ...
  it("previews and atomically clears import conflicts", () => {
    ...
    teams[1].slots[1].wheelIds[0] = wheel.id;
    ...
    expect(next[1].slots[0].awakenerId).toBeNull();
    expect(next[1].slots[1].wheelIds[0]).toBeNull();
    expect(next[1].posseId).toBeNull();
  });
});
